...
object Global extends Interaction {
  override def list: List[(Description, Symbol)] = {
  ...
  private def discoverClasses(cldr: ClassLoader, discovered: List[Class[_]] = Nil): List[Class[_]] = {
    cldr match {
  ...
  private def classesFromUrls(urls: Array[URL]) = {
    (for (
